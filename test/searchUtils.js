...
function findFormEventNames(){
    ...
    expect(inputParam[0], "expected custom event "+onInput+" to have the value typed in the input box").to.equal("some test query");
    
    expect(selectParam.length, "expected custom event "+onSelect+" to be fired with one parameter").to.equal(1);
    expect(selectParam[0], "expected custom event "+ onSelect+" to have the value chosen in the dropdown").to.deep.equal("some test type");
...
function findResultsEventName(){
    ...
    expect(dishChosen[0], "expected custom event "+onDishClick+" to be invoked with a search result").to.equal(searchResults[0]);
